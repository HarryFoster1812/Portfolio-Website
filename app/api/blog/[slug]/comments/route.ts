...
export async function GET(
  req: NextRequest,
  { params }: { params: { slug: string } } // ✅ Destructure `params` here
) {
  try {
    await connectDB();
  ...
  const { slug } = await params;
    ...
    const comments = await Comment.find({ slug: slug})
      ...
      .lean();
        ...
        console.log("db response:" + comments);
    // Structure into tree
    ...
    console.error(err);
    return NextResponse.json({ message: 'Error loading comments' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  if (!name || !body) {
    return NextResponse.json({ message: 'Missing name or comment body' }, { status: 400 });
  }
