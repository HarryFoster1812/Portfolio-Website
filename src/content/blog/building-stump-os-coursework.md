...
STUMP OS is a fully functional, from-scratch operating system (kinda) written in pure assembly for the University of Manchester’s STUMP teaching board which were 9 years old at the time of writing.
...
To begin I want to walk you through all of the limitations that were placed on us.

##  The ISA
The STUMP ISA (which I was given) is deliberately minimalistic. It provides only **six general-purpose registers** (R1–R6), with R0 hardwired to zero and R7 acting as the program counter, which severely limits the number of temporary values that can be held simultaneously. On top of this, since it is a 16-bit processor, immediate values are restricted to **5-bit two's complement constants** (-16 to 15). This made arithmetic and memory operations so much more challenging. The processor made in verilog, does not have a barrel shifter so the ISA supports only **single-shift operations per instruction**. There is a dedicated inaccessible flag register which is updated using the {s} variant of the arithmetic operations. This flag register is what the ISA uses to branch. There are 4 flags:
- Z (Zero)
- C (Carry)
- V (Overflow)
- N (Negative)
...
## Memory
STUMP is limited in memory, we are only given from 0x0000 to 0x1FFF to use, any more and the assembly run time environment we were using (bennett) would just zero whatever was written. The memory we uses was word sized (each address stored 16-bits) which was quite convenient and made memory limitations a bit easier.

## Peripherals and Hardware Limitations

In addition to the tight register and memory constraints, STUMP's peripheral and hardware design imposed further challenges. The board only provided a handful of peripherals which were memory-mapped from 0xFF00 onwards. We were given to the following peripherals:
- An LED Matrix (8x8 which with 8-bit colour, RRRGGGBB)
- LCD Screen (4x20)
- Buzzer (programmable for playing single notes)
- Keypad (0-9, # and * buttons)
- Switches (A-H with LED's on-top)
- Vibration motor (just on or off)
- Real-time clock
- A free running counter

All of the driver and implementations can be found here [STUMPOS/peripherals](https://github.com/HarryFoster1812/StumpOS/tree/master/Peripherals)

A major challenge came from **switch and keypad inputs**. The boards were 9 years old at the time, and electrical leakage often caused unreliable bit patterns. (eg if switch E was pressed it might also send switch D at the same time)
...
# Planning
Just like any project, the first step I took was planning. Armed with only my knowledge from the operating systems course unit last year, after countless hours of consulting with chatgpt, reading online discussions about OS dev and watching youtube videos I came up with a plan.
...
Now there are a lot of changes that I made from the first version of the plan. If you notice I wanted to use memory addresses higher than 1FFF which I later found out I could not. This forced me to rethink everything. I even considered switching projects because of  the memory constraints.
...
Now I knew what I needed to do I spent a day (and a few hours in the pub before a social) making malloc and free. 
...
When `free` is called, the block is marked as free, and the **next adjacent block** is checked to see if it can be merged, helping to reduce fragmentation. This is a very **simple coalescing strategy** a more sophisticated approach like eager coalescing would be a lot better but for the scope of this project, it was sufficient and kept the implementation manageable.

This was one of the very first pieces of code i wrote which is why it was originally written in C then translated into assembly. This was a good process but very time consuming which is why I opted to not do after this.
...
Here is my implementation of context switch:
```asm
...
When switching to kernel mode I pop the syscall return address making the head of the stack the user return address and switch to the kernel stack which looks like (at the end of the context switch)
...
# Syscalls
Now that we have a context switching and a heap, the kernel can safely execute functions that modify registers or memory without affecting user processes. However, we need a way for user programs to request these kernel services. This is where **system calls** come in.

A system call is a interface that allows a user program to request the operating system to perform a task on its behalf, something that the program cannot safely or directly do itself, such as allocating memory, writing to a display, or interacting directly with hardware. The system call mechanism ensures that the kernel can execute these operations in a privileged, isolated context, while maintaining the safety and state of the user process.

Here is my sys_call function:
```asm
sys_call:
...
First it called the context switch function which gives the requested function an isolated environment then it calculates the return address and calls the requested function and then switches back to the user process. 
...
One recurring problem was **peripheral management**. Since multiple processes could be running at the same time, I needed a way to ensure that only **one process could access a peripheral at once**. Without this, two processes could try to write to the same peripheral like the LCD screen.

This is where I borrowed the idea of **mutexes**. A mutex (mutual exclusion) is a simple synchronisation mechanism that allows **exclusive ownership** of a resource. At any given time, only one process can hold the mutex for a peripheral, and all other processes must wait until it is released. In StumpOS, this provided a straightforward way to arbitrate access to hardware devices and prevent processes from interfering with each other.
...
This also relies upon co-operation to make this system effective. In each process at the start of the execution loop the process must  check if it has access to the peripherals before using them:
...
- The **current page number** (for example, _Page 01 of 05_)
